...
- **Entry filters (regime):** HTF confirm + EMA separation + NoChop
...
1. **htf_confirm** — HTF SMA confirm (forward-filled to 5m)
   - params: `fast=30`, `slow=75`

2. **ema_sep** — HTF EMA separation filter (ATR-scaled)
   - params: `ema_fast=40`, `ema_slow=300`, `atr_n=20`, `sep_k=0.05`

3. **nochop** — HTF NoChop filter
...
6. **seasonality_cap** — month-based size cap
   - params: `month_size_cap: {6: 1.0}`

7. **churn** — debounce + cooldown (trade frequency control)
   - params: `min_on_bars=3`, `cooldown_bars=8`

8. **mid_loss_limiter** — mid-duration loss kill-switch
   - params: `min_bars=13`, `max_bars=48`, `stop_ret=-0.010`

9. **no_recovery_exit** — no-recovery exit
   - params: `bar_n=24`, `min_ret=-0.005`

10. **shock_exit** — shock day kill-switch (+ optional cooldown)
