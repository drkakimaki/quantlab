...
def main() -> None:
    ...
    xau5_2022 = load_ohlc_from_daily_parquet(symbol=symbol, start=p1_start, end=p1_end, root=root_5m_ohlc)
    xau5_2325 = load_ohlc_from_daily_parquet(symbol=symbol, start=p2_start, end=p2_end, root=root_5m_ohlc)
    xau5_2026 = load_ohlc_from_daily_parquet(symbol=symbol, start=p3_start, end=p3_end, root=root_5m_ohlc)

    xag5_2022 = load_ohlc_from_daily_parquet(symbol=corr_symbol, start=p1_start, end=p1_end, root=root_5m_ohlc)
    xag5_2325 = load_ohlc_from_daily_parquet(symbol=corr_symbol, start=p2_start, end=p2_end, root=root_5m_ohlc)
    xag5_2026 = load_ohlc_from_daily_parquet(symbol=corr_symbol, start=p3_start, end=p3_end, root=root_5m_ohlc)

    xau15_2022 = load_ohlc_from_daily_parquet(symbol=symbol, start=p1_start, end=p1_end, root=root_15m_ohlc)
    xau15_2325 = load_ohlc_from_daily_parquet(symbol=symbol, start=p2_start, end=p2_end, root=root_15m_ohlc)
    xau15_2026 = load_ohlc_from_daily_parquet(symbol=symbol, start=p3_start, end=p3_end, root=root_15m_ohlc)

    px_2022 = xau5_2022["close"].astype(float)
    px_2325 = xau5_2325["close"].astype(float)
    px_2026 = xau5_2026["close"].astype(float)

    cc_2022 = xag5_2022["close"].astype(float)
    cc_2325 = xag5_2325["close"].astype(float)
    cc_2026 = xag5_2026["close"].astype(float)
        ...
        for a in min_abs:
            for flb in flip_lb:
                for mf in max_flips:
                    ...
                    # We care about winrate + CAGR, but keep DD under control.
    ...
    out = Path("reports/trend_variants/xagusd_filters/corr_filter_search.csv")
