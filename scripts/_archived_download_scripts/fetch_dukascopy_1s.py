...
from quantlab.data.dukascopy import SYMBOL_SPECS, build_daily_1s, iter_days, latest_utc_date
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Download Dukascopy tick data and build 1-second spot quotes.")
    ap.add_argument("--symbols", nargs="+", default=["XAUUSD", "XAGUSD"], help="e.g. XAUUSD XAGUSD")
    ...
    print(f"Building 1s quotes for {args.symbols} from {args.start} to {end} ({total_days} days)")
    print(f"cache: {args.cache_dir} | out: {args.out_dir}")

    for sym in args.symbols:
        print(f"\n== {sym} ==")
        for i, day in enumerate(iter_days(args.start, end), start=1):
            out_path = build_daily_1s(
                symbol=sym,
                day=day,
